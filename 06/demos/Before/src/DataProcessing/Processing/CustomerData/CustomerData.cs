...
internal class CustomerData
{
    public static async Task ProcessCustomerDataSamplesAsync(ProcessingOptions options, CancellationToken cancellationToken = default)
    {
        ...
        {
            var countryWriter = new CustomerDataUniqueCountriesWriter(options, CultureInfo.InvariantCulture);
            await countryWriter.WriteAsync("dk-unique-countries.txt", customerData.PriorityCustomers,
                cancellationToken);
        }
    }
}